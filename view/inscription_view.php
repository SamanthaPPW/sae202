...
function index()
{
    ...
    if (isset($_GET['error'])) {
        if ($_GET['error'] === 'invalid_credentials') {
            ...
            // message affiché directement dans la vue
        } elseif ($_GET['error'] === 'email_already_exists') {
            $error_message = 'Cet email est déjà utilisé. Veuillez vous connecter ou utiliser un autre email.';
...
function verif_connexion()
{
    ...
    // Tout est bon, démarrage session
    session_start();
...
function validation_inscription()
{
    ...
    if ($token !== false) {
        $url = "https://ton-site.com/confirmation?token=$token";
